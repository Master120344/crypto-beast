...
const pancakeSwapFactory = new ethers.Contract(PANCAKESWAP_FACTORY, FACTORY_ABI, providerBSC);
...
// Fetch PancakeSwap BTC/BNB price
async function fetchPancakeSwapPrice() {
    try {
        const pairAddress = await pancakeSwapFactory.getPair(WBNB, BTCB);
        const pairContract = new ethers.Contract(pairAddress, PAIR_ABI, providerBSC);
        const reserves = await pairContract.getReserves();
        const reserveWBNB = Number(ethers.formatUnits(reserves.reserve0, 18));
        const reserveBTCB = Number(ethers.formatUnits(reserves.reserve1, 18));
        ...
        log(`PancakeSwap price fetch error: ${e.message}`);
    }
}
...
function connectKrakenWebSocket() {
    ...
    krakenWsClient.on('message', (data) => {
        try {
            ...
            if (Array.isArray(message) && message[1]?.ticker) {
                const ticker = message[1].ticker;
